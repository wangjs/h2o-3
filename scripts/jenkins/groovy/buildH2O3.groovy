def call(buildConfig) {
  ...
  withCustomCommitStates(scm, buildConfig.H2O_OPS_TOKEN, "${buildConfig.getGitHubCommitStateContext(stageName)}") {
    buildConfig.addStageSummary(stageName)
    buildConfig.setStageNode(stageName, env.NODE_NAME)
    try {
      ...
      buildConfig.markStageSuccessful(stageName)
      echo "************************${buildConfig.getBuildSummary()}***************************"
    } catch (Exception e) {
      buildConfig.markStageFailed(stageName)
      throw e
    }
  }
}
